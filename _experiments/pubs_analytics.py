    ...
    for experiment, inst_map in by_instance.items():
        ...
        for inst, metrics in inst_map.items():
            if not isinstance(metrics, dict):
                continue
            counts[(experiment, inst)] = int(metrics.get("nonminimal", 0) or 0)
    return counts


def latex_cell(value: object, nonminimal_count: int) -> str:
    if pd.isna(value):
        return r"\expna"
    ...
    if is_bold:
        text = text[:-1]
    rendered = text if nonminimal_count == 0 else f"{text} [{nonminimal_count}]"
    return rf"\textbf{{{rendered}}}" if is_bold and nonminimal_count == 0 else rendered
...
def make_row(row: pd.Series, nonminimal_counts: dict[tuple[str, str], int]) -> str:
    ...
    cells = [
        latex_cell(row.get(inst, ""), nonminimal_counts.get((experiment, inst), 0))
...
def build_tabular(df: pd.DataFrame, nonminimal_counts: dict[tuple[str, str], int]) -> str:
    lines = [
    ...
    for tmax_idx, tmax in enumerate(tmax_values):
        block = ordered[ordered["parsed_tmax"] == tmax]
        for _, row in block.iterrows():
            lines.append(make_row(row, nonminimal_counts))
        if tmax_idx != len(tmax_values) - 1:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="T",
        help="Read verification counts from <root_dir>/_results/verify_control_T<T>.json",
    )
    ...
    nonminimal_counts: dict[tuple[str, str], int] = {}
    if args.tmax_upper is not None:
        verify_path = root_dir / "_results" / f"verify_control_T{args.tmax_upper}.json"
        if not verify_path.exists():
            raise FileNotFoundError(f"Missing verification file: {verify_path}")
        nonminimal_counts = load_verification_counts(verify_path)

    latex = build_tabular(df, nonminimal_counts)
